...
def show_learning_sample_random(inputs, values, output_value, output_detections, output_points, mean, std, waitTime=0):
    ...
    image = cv2.putText(image, str(int(value*52))+"-"+str(int(target_value*52)), org, font, fontScale, color, thickness, cv2.LINE_AA)
...
# TODO: Fix the issue with denormalisation?
def show_mask_points_sample_batch(inputs, values, output_value, output_detections, output_points, output_masks, mean, std, waitTime=0):
    global points_colours, font, org, fontScale, color, thickness
...
# TODO: Fix the issue with denormalisation?
def show_points_from_mask_sample_batch(inputs, values, output_value, output_detections, output_masks, mean, std, threshold=0.1, waitTime=0):
    global points_colours, font, org, fontScale, color, thickness
...
def show_model_validation(model, validation_loader, mean, std, waitTime=0):
    ...
    for batch_index, batch in enumerate(validation_loader):
        ...
        show_points_from_mask_sample_batch(inputs, value, out_value, out_detections, out_mask, mean, std, waitTime=waitTime)
